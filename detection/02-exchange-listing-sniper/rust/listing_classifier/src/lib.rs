...
/// Classify an exchange listing announcement title via the C FFI boundary.
///
/// # Safety
///
/// `exchange` and `title` must be valid, NUL-terminated C strings, and `out`
/// must point to a writable, properly aligned [`NativeListingResult`]. Passing
/// dangling, misaligned, or non-NUL-terminated pointers is undefined behavior.
/// Null pointers are handled explicitly and cause the function to return `-1`.
#[no_mangle]
