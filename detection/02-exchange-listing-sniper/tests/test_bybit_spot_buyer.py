...
from src.bybit_spot_buyer import BybitSpotBuyer  # noqa: E402
from src.cpp_fast_buyer import CppFastBuyerBridge  # noqa: E402
...
class _ShortBulkFastExecutor:
    ...
    def buy_market(self, *, symbol, quote_amount, order_link_id):
        raise AssertionError("bulk path should not call single buy_market")

    def buy_market_quote_text(self, *, symbol, quote_amount_text, order_link_id):
        raise AssertionError("bulk path should not call single buy_market_quote_text")

    def buy_markets_quote_text(self, *, orders, quote_amount_text):
        self.bulk_orders = list(orders)
...
class _LegacySingleFastExecutor:
    def __init__(self):
        self.orders = []
    ...
    def buy_market(self, *, symbol, quote_amount, order_link_id):
        self.orders.append((symbol, quote_amount, order_link_id))
        return {
            "attempted": True,
            "executed": True,
            "ret_code": 0,
...
def test_explicit_empty_credentials_override_env(monkeypatch):
    ...
    result = buyer.buy_market(ticker="STRK", order_link_id="ls-test-empty")
...
def test_cpp_only_bulk_buy_uses_fast_bulk_and_pads_missing_responses():
    ...
    assert fast_executor.bulk_orders == [
        ("SENTUSDT", "ls-test-sent"),
        ("ELSAUSDT", "ls-test-elsa"),
    ]
    assert [item["symbol"] for item in results] == ["SENTUSDT", "ELSAUSDT"]
    ...
    assert results[1]["order_link_id"] == "ls-test-elsa"


def test_cpp_only_bulk_buy_falls_back_to_single_executor_when_bulk_api_missing():
    fast_executor = _LegacySingleFastExecutor()
    buyer = _buyer(
        api_key="key",
        api_secret="secret",
        buy_enabled=True,
    ...
    assert fast_executor.orders == [
        ("SENTUSDT", 10.0, "ls-test-sent"),
        ("ELSAUSDT", 10.0, "ls-test-elsa"),
    ]
    assert [item["symbol"] for item in results] == ["SENTUSDT", "ELSAUSDT"]
...
def test_cpp_fast_bridge_parses_bulk_response_lines():
    response = (
        b"BULK\t2\n"
        b"BUY\t1\t1\tSENTUSDT\torder-sent\t0\tcpp_fast_path\t\n"
        b"BUY\t0\t1\tELSAUSDT\t\t10001\tcpp_fast_path\tinsufficient balance"
    )

    results = CppFastBuyerBridge._parse_bulk_buy_response(
        response,
        [("SENTUSDT", "ls-test-sent"), ("ELSAUSDT", "ls-test-elsa")],
        "10",
    )

    assert [item["symbol"] for item in results] == ["SENTUSDT", "ELSAUSDT"]
    assert [item["requested_usdt"] for item in results] == [10.0, 10.0]
    assert results[0]["executed"] is True
    assert results[1]["executed"] is False
    assert results[1]["reason"] == "insufficient balance"
