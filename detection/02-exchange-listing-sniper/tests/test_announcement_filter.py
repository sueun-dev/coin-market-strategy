...
from src import announcement_filter as announcement_filter_module
from src.announcement_filter import (
    classify_listing_title_python,
    extract_listing_assets,
    make_listing_title_classifier,
)
from src.native_classifier import NativeClassifierBackend, _native_library_paths
from src.native_classifier import NativeClassifierManager
...
def test_default_classifier_falls_back_to_python_when_native_backend_misses(monkeypatch):
    monkeypatch.setattr(
        announcement_filter_module,
        "get_native_classifier_manager",
...
def test_native_manager_ignores_stale_rust_classifier_when_present():
    ...
    assert ("rust" in names) is manager._backend_matches_semantic_canaries(backend)
