...
def test_listing_matched_post_filters_invalid_native_trade_payloads():
    post = _build_listing_matched_post(
        payload={
            "@type": "listingMatched",
            "channel_handle": "BithumbExchange",
            "message_id": 321991,
            "published_at_unix": 1778680004,
            "title": "[마켓 추가] 월드 리버티 파이낸셜(WLFI), 밈코어(M) 원화 마켓 추가",
            "ticker": "WLFI",
            "tickers": ["WLFI", "M"],
            "native_trade": {"ticker": "WLFI", "reason": "tdlib_native_rest_preflight"},
            "native_trades": [
                {"ticker": "WLFI", "reason": "tdlib_native_rest_preflight"},
                "not-a-trade",
                {"ticker": "M", "reason": "tdlib_native_rest_preflight"},
        ...
        event_received_monotonic_ns=25_000,
        clock_offset_ns=0,
    )

    assert post["native_trade"]["ticker"] == "WLFI"
    assert [trade["ticker"] for trade in post["native_trades"]] == ["WLFI", "M"]
